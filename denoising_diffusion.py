...
def show_batch(input_batch, count=0, grayscale=False, normalize=True):
	...
	for n in range(min(batch_size, 4)):
	...
	plt.tight_layout()
	plt.savefig('image{0:04d}.png'.format(count), dpi=310, transparent=True)
	plt.close()
	return
...
# 		loss = p_ losses(model, batch, timestep, loss_type="l2")
...
# 	torch.save(model.state_dict(), 'landscapes_256_diffusion.pth')


gen_images = sample(model, image_size, batch_size=2, channels=channels)
show_batch((gen_images[-1] + 1) / 2, count=0, grayscale=False)
